...
def get_model_location(run_id):
    """Get the model location from environment variables or construct it based on the run_id."""
    model_location = os.getenv('MODEL_LOCATION')

    if model_location is not None:
        return model_location

    model_bucket = os.getenv('MODEL_BUCKET', 'mlops-storage-practice')
...
class ModelService:
    ...
    def prepare_features(self, ride):
        ...
        cols = ['Open', 'Volume']
        features = pd.DataFrame([{k: ride[k] for k in cols}])
        return features
